...
def landmark_to_servo_angle(hand_landmarks):
    ...
    # Apply low-pass filter for servo x
    servo_angle[0] = alpha_x * map_range(angle, palm_angle_min, palm_angle_mid, x_max, x_min) + \
                     (1 - alpha_x) * prev_servo_angle[0]


    # Use palm size as z angle
    palm_size = clamp(palm_size, plam_size_min, plam_size_max)
    # Apply low-pass filter for servo z
    servo_angle[1] = alpha_z * map_range(palm_size, plam_size_min, plam_size_max, z_max, z_min) + \
                     (1-alpha_z) * prev_servo_angle[1]
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:            
                if len(results.multi_hand_landmarks) == 1:
                    ...
                    # Apply low-pass filter for servo x angle output
                    servo_x_angle = alpha_x * servo_x_ctrl_simulation.output['servo_x'] + \
                                    (1-alpha_x) * prev_servo_angle[0]
                    ...
                    prev_servo_angle = servo_angle
                    servo_angle = [int(angle) for angle in servo_angle]
                    if not debug:
